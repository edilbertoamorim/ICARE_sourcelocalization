function [x] = load_eeglab_eeg(filepath, use_saved, save_result)
...
%        this script creates it.
    ...
    [status, ~, ~] = mkdir(Config.get_configs('output_dir'), 'matfiles');
    assert(status==1);
    
    eeglab_save_folder = fullfile(Config.get_configs('output_dir'), 'matfiles/');
    eeglab_save_path = fullfile(eeglab_save_folder, patient_id, [filename_no_ext, '.mat']);
    ...
    if use_saved && exist(eeglab_save_path, 'file')==2
        ...
        t = tic;
        eeglab_struct = load(eeglab_save_path, '-mat', 'x');
        toc(t);
        x = eeglab_struct.('x');
        % Check if the loaded struct actually contains data
        is_loaded_good = check_loaded_eeglab(x);
    ...
    if requires_load
        ...
        x = struct_to_EEGLab(standardized_struct, SUB_EPOCH_LENGTH);
