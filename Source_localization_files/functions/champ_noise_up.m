...
% nt = # of data time points
    ...
    function [gamma,x,w,c,l, sigu]=champ_noise_up(y,f,sigu,nem,nd,vcs,plot_on,coup,noup,ncf, tol);

if vcs==2 && nd>1
  [gamma x w l v c sigu]=champ_mat(y,f,sigu,nem,nd,plot_on,coup,noup,ncf, tol);
else
  [gamma x w l v c sigu]=champ_vec(y,f,sigu,nem,nd,vcs,plot_on,coup,noup,ncf, tol);
end
...
invf2(ff)=1./f2(ff);
f=double(f);
w=spdiags(invf2',0,nvd,nvd)*f';%invL %*% L
...
for iem=1:nem
%    iem
    vmat=diag(vvec);%spdiags(vvec,0,nvd,nvd);
...
%    [p d q]=svd(c);
    ...
    invd(ff)=1./d(ff);   %concave function parameters 1/a1,1/a2,....1/aN %used for 4.74
      invc=p*diag(invd)*p';          %model data convariance y %used for 4.74
    invc = inv(c); %var(Y)^(-1)
...
%    like(iem)=-.5*(sum(log(d))+nk*log(2*pi))-.5*sum(sum(y.*(invc*y)))/nt; 
    ...
    if iem >1
      if abs(like(iem) - like(iem-1))/nt <=tol
        break
      end
    end
    ...
    if(plot_on)
    subplot(2,2,1);plot((1:iem),like(1:iem), 'LineWidth', 2);
    title('Learned Champ.');
    xlabel('iteration');
    set(gca(),'XLim',[0 iem]);
    end
    
    fc=f'*invc;         %4.82 TL * inv(covY)
...
%         end
    ...
    if(plot_on)
    subplot(1,2,2);plot((1:nv),v, 'LineWidth', 2);
    title('Voxel power');
    xlabel('voxel index');
    set(gca(),'XLim',[1 nv]);
    drawnow
...
function [gamma,x,w,like,vvec,c,sigu]=champ_mat(y,f,sigu,nem,nd, ...
                                                plot_on,coup,noup, ncf, tol);
