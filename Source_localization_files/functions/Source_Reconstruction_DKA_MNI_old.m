...
% PSDs mat files.
    ...
    % c.Features
    feature_resolution = 10;     % features time resolution [sec]

    % c.Reconstruction
    champ_iter = 80;                  %[Champagne iterations]
    n_dir = 3;                        %[Reconstruction directions (x,y,z)]
    reconstruction_resolution = 5;    %[min] (PSDs - Signal time window [minutes])
    ...
    for i_file = start_file_idx:length(eegFiles)
        ...
        segment_samples = reconstruction_resolution * 60 * Fs_acquisition;
        n_segments = floor(size(signal,1) / segment_samples);

        % If max_min set, process only max_min [minutes] from the start of file
        if max_min
            max_nSeg = min(max_min, n_segments*reconstruction_resolution); 
        end
        ...
        for s = p_start:max_nSeg
              ...
              try 
                ...
                idxStart = (s-1)*segment_samples + 1;
                idxEnd   = s*segment_samples;
                ...
                PSD_data.psd_resolution = reconstruction_resolution;
                PSD_data.CPC = CPC;
